...
> Status: Repo-side lanes A-F complete and gate-green; release remains
>   operator-gated on live evidence
> Owner: VHC Core Engineering + VHC Launch Ops
> Current main: `d5ddd89e00384749d88e7404feaa2046a706471c`
...
Release itself stays operator-gated. The remaining work is not repo code: it is
live evidence regeneration at the release commit, the operator-owned Scope A
canary that turns accepted synthesis on for A6, live provider registration and
callback-boundary deployment, and the manual multi-client rehearsal. Those are
enumerated below and must not be shortcut.
...
## Repo-Side Gate Status (main `d5ddd89e`)
...
`check:luma:mvp-production-readiness` remains `blocked` on exactly two
evidence-flow items, both operator/release-time and neither a repo-code gap:

1. `repo_dirty` / `repo_clean` — resolves on a clean checkout at the release
   commit (the local tree carries only untracked operator planning docs and
   `.tmp` artifacts).
2. `mesh_luma_coverage` — the Mesh evidence packet is pinned to an older commit
   and must be regenerated at the release commit
   (`tools/scripts/regenerate-mvp-release-evidence.mjs`) against live mesh.
...
- **Scope A canary (Slice A3).** Enabling accepted synthesis on A6 is an
  ...
  on A6.
- **Mesh evidence regeneration** at the release commit on a clean tree, to
  clear `mesh_luma_coverage`.
