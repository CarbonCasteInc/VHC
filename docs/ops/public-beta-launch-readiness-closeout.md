...
Operational recovery note, 2026-05-31: PR #631 restored the deployed public REST feed path from the StoryCluster-derived repair payload and deployed relay snapshot serving for `venn.carboncaste.io`. Current recovery evidence from `.tmp/release-evidence/public-feed-composition-freshness/1780223335637/public-feed-composition-freshness-summary.json` shows the origin latest-index returns 28 product-visible stories with live story-body readback for every sampled row; snapshot serving verifies body readback before visibility so stale snapshot rows cannot remain visible when their story route is missing. The visible 28-row window includes 22 singleton and 6 multi-source/corroborated rows. The 80-row public browser-smoke relay readback in `.tmp/release-evidence/public-feed-browser-smoke/1780224995446/public-feed-browser-smoke-summary.json` reports 59 singleton and 21 multi-source/corroborated stories, all `synthesis_pending`, with synthesis reads returning 404 for 80/80 sampled rows. That same smoke run proves public app open populates without a manual refresh, refresh preserves the mixed feed, and scroll/load-more issues `refreshLatest({ limit: 15, before: 1780190425000 })`, grows the store from 15 to 30 stories, and renders 30 visible cards from a non-overlapping older relay cursor page. It still fails release readiness on accepted synthesis visibility: direct Gun readback exposes a stale accepted-synthesis candidate for `story-d4197fa60cbf`, but public relay lifecycle marks the current story/source-set revision as pending and the deployed app does not render an accepted synthesis/frame table. This recovery packet is therefore not release-ready evidence. Fresh RSS ingest-to-feed propagation, synthesis worker catch-up/current accepted synthesis, frame/reframe point-id readiness, stance/aggregate/decay public-mesh convergence, and direct Gun signature cleanup remain blocking or unproven. Snapshot-backed relay recovery is allowed only as explicit operational repair evidence and must not be described as release readiness unless all configured public peers, fresh ingest, synthesis, browser, lifecycle, and stance/aggregate gates pass.
