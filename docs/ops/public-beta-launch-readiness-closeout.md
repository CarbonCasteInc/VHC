...
Operational recovery note, 2026-05-31: PR #631 restored the deployed public REST feed path from the StoryCluster-derived repair payload and deployed relay snapshot serving for `venn.carboncaste.io`. Current recovery evidence from `.tmp/release-evidence/public-feed-composition-freshness/latest/public-feed-composition-freshness-summary.json` shows the origin latest-index returns 28 product-visible stories with live story-body readback for every sampled row; snapshot serving verifies body readback before visibility so stale snapshot rows cannot remain visible when their story route is missing. The visible feed includes 22 singleton and 6 multi-source/corroborated rows, with non-overlapping cursor pagination and one multi-source story reporting `accepted_synthesis_available` plus `frame_table_ready` with 5/5 frame point ids and 5/5 reframe point ids. The public composition gate passes latest-index and sampled story-body readback across `gun-a.carboncaste.io`, `gun-b.carboncaste.io`, and `gun-c.carboncaste.io`; all configured public relay origins currently expose the same mixed latest-index window after durable relay data reseed from the healthy public peer. Manual public-app browser evidence in `.tmp/release-evidence/public-app-browser-smoke/manual-20260531-v27-final/` proves app open populates without a manual refresh, refresh preserves the mixed feed, and scroll/load-more fetches an older cursor window from the mesh instead of revealing a larger preloaded DOM window. The gate records honest `synthesis_pending` lifecycle rows without re-scraping article text, because product visibility must not require accepted synthesis. This is still not release-ready evidence by itself: fresh RSS ingest-to-feed propagation, direct Gun lifecycle accountability, automated browser-smoke release gating, and stance/aggregate/decay public-mesh convergence remain unproven in this recovery packet. Snapshot-backed relay recovery is allowed only as explicit operational repair evidence and must not be described as release readiness unless all configured public peers, fresh ingest, synthesis, browser, lifecycle, and stance/aggregate gates pass.
