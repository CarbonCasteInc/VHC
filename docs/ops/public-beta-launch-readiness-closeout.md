...
| `public_feed_lifecycle_accountability` | Raw `vh/news/stories`, product latest/hot indexes, relay latest-index, synthesis lifecycle, and accepted synthesis paths agree; eligible raw stories are not hidden because synthesis is pending; hot index rows exist for product-visible stories and carry current source-set product metadata; stale topic latest synthesis from an older source-set revision is not counted as accepted/current. | `pass` |
