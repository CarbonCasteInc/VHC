...
Operational recovery note, 2026-05-31: PR #631 restored the deployed public REST feed path from the StoryCluster-derived repair payload and deployed relay snapshot serving for `venn.carboncaste.io`. Current recovery evidence shows the origin latest-index returns 28 visible stories, including 22 singleton and 6 multi-source/corroborated rows, with non-overlapping cursor pagination. The latest public composition gate now passes latest-index and sampled story-body readback across `gun-a.carboncaste.io`, `gun-b.carboncaste.io`, and `gun-c.carboncaste.io`; `gun-c` required a local runtime wrapper repair and a 28-row durable story/latest/lifecycle seed. The gate records `not_checked_synthesis_pending` for honest pending lifecycle rows instead of re-scraping article text, because product visibility must not require accepted synthesis. This is not release-ready evidence by itself: the recovered rows are all `synthesis_pending`, accepted synthesis/frame-table/stance gates remain unproven, and fresh RSS ingest-to-feed propagation was not re-proven in this run. Snapshot-backed relay recovery is allowed only as explicit operational repair evidence and must not be described as release readiness unless all configured public peers, fresh ingest, synthesis, and stance/aggregate gates pass.
