...
| `public_feed_pagination_refresh` | Public app opens current stories, refreshes latest stories, and load-more/scroll performs an older-window mesh refresh with an exclusive cursor; revealing rows from a larger initial DOM or in-memory window does not satisfy this gate. | `pass` |
