...
3. update the source surface only if the source passes the readable-source contract;
4. run the blocking StoryCluster correctness and daemon-first browser gates;
5. record source-level evidence in the PR summary or lane note;
6. if the source fails later in production-like testing, remove or disable it rather than weakening the readability contract.

## Required Validation

Source-surface changes should keep these in the loop:

1. `pnpm test:storycluster:correctness`
2. `pnpm test:storycluster:gates`
