...
> Date: 2026-04-20
> Last alignment audit: 2026-04-26 during public-beta compliance minimums branch from `main` at `5ea6558c`
...
| 7 | `compliance-public-beta-minimums` | Complete for Web PWA beta policy surfaces. | Privacy, terms, UGC/moderation, support/contact, data deletion, telemetry/remote AI consent, and content/copyright boundaries. | `/beta`, `/privacy`, `/terms`, `/moderation`, `/support`, `/data-deletion`, `/telemetry`, and `/copyright` exist and are linked from the app; `pnpm check:public-beta-compliance` verifies required routes, links, docs, and no-overclaim language, and `pnpm check:mvp-release-gates` includes `public_beta_compliance`. |
...
| Compliance | Go for public beta policy surfaces; legal review and operator contact provisioning remain separate release approvals. | Privacy, terms, UGC/moderation, support/contact, data deletion, telemetry/remote AI consent, and content/copyright minimums have accepted drafts, user-facing routes, global links, and `pnpm check:public-beta-compliance` / `public_beta_compliance` MVP-gate coverage. | No native App Store/TestFlight claim, legal approval claim, verified-human claim, or full trust-and-safety operations claim. |
...
Still required: regeneration workflow polish, legal review, trust-gated operator roles, notifications/escalation policy, broader admin workflow UX, and a provisioned support/contact channel for any public cohort beyond operator-invited beta users.
