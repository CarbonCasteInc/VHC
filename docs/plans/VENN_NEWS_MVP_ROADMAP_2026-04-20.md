...
| Scope A public-news operation | Phase 5 Scope A is recovered/fresh on A6 after outage #2 with raw-only publication, 2-of-3 relay REST quorum, graph diagnostics, early heap capture wiring, fresh-bundle priority, and staggered relay watchdog ceilings. The 2026-07-03 driver verdict is `heap_driver_off_graph_likely`: graph live bytes, tombstones, and soul counts do not explain heap growth, and the missing early-capture artifact was threshold-not-reached. | Do not treat outage recovery as sustained-readiness proof. The next Scope A PR is early-capture threshold retuning plus secret-safe retainer identification. Retention, publisher clear, eviction, and relay compaction are out of scope until retainer evidence names them. A6 alert enablement and the `2026-07-03T13:04Z` relay restart cause remain operator-owned readiness items. |
