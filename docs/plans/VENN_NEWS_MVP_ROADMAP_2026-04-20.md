...
> Date: 2026-04-20
> Last alignment audit: 2026-04-28 during full-product multi-user engagement docs alignment from `main` after PR #550, carried by PR #551
...
| Release gates | `pnpm check:mvp-release-gates` now composes `source_health`, `story_correctness`, deterministic Web PWA feed/detail/stance/thread/moderation smokes, the curated launch-content snapshot gate, the report-intake/admin-action smoke, the operator trust gate, the public-beta compliance gate, and `public_beta_launch_closeout`. It writes `.tmp/mvp-release-gates/latest/mvp-release-gates-report.json` with per-gate `pass`, `fail`, `setup_scarcity`, or `skipped_not_in_scope` status. `pnpm check:public-beta-compliance` is also available as a focused command for policy route/docs/support-channel/private-escalation checks. `pnpm check:public-beta-launch-closeout` verifies the final closeout evidence map in `docs/ops/public-beta-launch-readiness-closeout.md`. | Week 3A release evidence harness and final closeout audit are present for the core news loop, curated fallback content, minimum report-to-action workflow, trusted beta operator gate, public-beta policy surfaces, public support/contact path, and private support escalation protocol; broader ops/admin polish is classified as post-beta follow-up unless release copy claims production-grade live headline readiness. |
