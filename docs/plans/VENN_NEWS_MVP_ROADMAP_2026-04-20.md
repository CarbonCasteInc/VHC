...
> Date: 2026-04-20
> Last alignment audit: 2026-04-26 during docs alignment after public-beta support/contact merge from `main` at `6745c26a`
...
| Compliance | Go for public beta policy surfaces and provisioned support/contact; legal review remains a separate release approval. | Privacy, terms, UGC/moderation, support/contact, data deletion, telemetry/remote AI consent, and content/copyright minimums have accepted drafts, user-facing routes, global links, a VHC public beta GitHub Issue Form support channel, and `pnpm check:public-beta-compliance` / `public_beta_compliance` MVP-gate coverage. | No native App Store/TestFlight claim, legal approval claim, private support inbox claim, verified-human claim, or full trust-and-safety operations claim. |
...
| Correction/admin path | Go for minimum MVP remediation controls; accepted synthesis correction, story-thread comment hide/restore moderation, and report intake/admin action queue are implemented. | Users can report accepted synthesis artifacts and story-thread comments; operators can dismiss reports or apply existing suppress/unavailable/hide/restore actions with typed audit metadata and `source_report_id` provenance; story detail hides stale summary/frame rows and moderated comment content; `pnpm check:mvp-release-gates` includes deterministic `synthesis_correction`, `story_thread_moderation`, and `report_intake_admin_action` smokes. | Public launch still needs trust-gated operator roles, private escalation handling, notification/escalation workflow, and broader admin UX polish; do not imply a full trust-and-safety operations console exists. |
...
Still required: regeneration workflow polish, legal review, trust-gated operator roles, notifications/escalation policy, broader admin workflow UX, private escalation handling for support requests that cannot be resolved in public GitHub issues, and model/cost operations visibility.
