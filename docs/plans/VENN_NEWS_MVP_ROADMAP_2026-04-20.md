...
> Date: 2026-04-20
> Last alignment audit: 2026-07-03 during MVP readiness state alignment after #701
> Target: Four-week Web PWA MVP launch path after the release commit evidence packet passes and any external release approvals are recorded
...
| Story discussion UI | PR #533 is merged into `main`. Story detail uses deterministic `news-story:<encoded story-or-topic token>` headline thread ids, resolves exact deterministic matches before legacy topic/source matches, renders the thread below the frame/reframe table, and exposes recoverable load/create/post errors. Story-thread comment hide/restore moderation now has a typed audit path and deterministic gate coverage. Story-thread comments can be reported into the operator queue and actioned through the existing audited hide/restore records. Current remediation writes are protected by minimum trusted beta operator authorization. | W2 story-thread rendering, reply composer, minimum audited hide/restore moderation, minimum report-to-action workflow, and current-action operator trust gate are implemented at MVP level. Remaining thread work is user block UX, full RBAC/admin membership management, notification/escalation automation, appeals, and richer operator workflow polish. |
| Public beta policy/support | PR #545, PR #546, PR #548, and PR #549 are merged into `main`. `/compliance`, `/beta`, `/privacy`, `/terms`, `/moderation`, `/support`, `/data-deletion`, `/telemetry`, and `/copyright` are linked from the Web PWA footer; `/support` links to the VHC public beta GitHub Issue Form backed by `.github/ISSUE_TEMPLATE/public-beta-support.yml`; `pnpm check:public-beta-compliance` verifies route/docs/support-channel wiring, operator trust-gate wiring, no-overclaim boundaries, and private escalation protocol coverage. | Web PWA public-beta policy surfaces, a reachable public support/contact path, minimum private handoff protocol, and minimum trusted beta operator gate for current remediation actions are implemented. This is not legal approval, a private support desk, full RBAC/admin membership management, notifications/escalation automation, SLA handling, or a full trust-and-safety operations console. |
| LUMA public-beta identity | Public-beta identity creation now uses a persisted beta-local `AssuranceEnvelope`, stable device credential compartments, session lifecycle enforcement, signed-write `sessionRef` values, a centralized MVP action policy for directory/forum/news-report/aggregate/stance write actions, M1.B identity controls/UI, M1.C profile hardening, M1.D forbidden-claim gating, and M1.E telemetry source-discipline/fixture replay. This is still not production-attestation/Silver, verified-human identity, one-human-one-vote, Sybil resistance, cryptographic residency, full `§21.4` recorded product replay, or `<TrustClaim>` readiness. | `identity-honesty-scope` resolves the Web PWA beta path: copy must say beta-local identity/proof semantics. `pnpm check:luma:mvp-production-readiness`, `pnpm check:luma-forbidden-claims`, `pnpm check:luma-production-profile`, and `pnpm check:luma-telemetry-redaction` are the focused readiness/honesty gates; public WSS mesh `release_ready`, `<TrustClaim>`, and full production app readiness remain separate gates. |
| Scope A public-news operation | Phase 5 Scope A is fresh on A6 after Slice 0 alert enablement and the post-Slice-0 stale-feed recovery. `main@47ba218d` carries the raw-only publication path, 2-of-3 relay REST quorum, graph diagnostics, staggered early heap capture, fresh-bundle priority, staggered relay watchdog ceilings, interim email alerting, watch closure, and #723 StoryCluster production-timeout bounding. The 2026-07-03 driver verdict is still `heap_driver_off_graph_likely`: graph live bytes, tombstones, and soul counts do not explain heap growth. | Do not treat outage recovery as sustained-readiness proof. The current Scope A posture is evidence accrual: leave publisher/relays untouched while green, let the clean window run from `2026-07-06T22:44:08.567Z`, and wait for the first post-recovery 500 MB -> 700 MB secret-safe heap-summary pair. Retention, publisher clear, eviction, relay compaction, custom pager cutover, and Codex live execution are out of scope until their separate evidence gates pass. |
| Release gates | `pnpm check:mvp-release-gates` now composes `source_health`, `story_correctness`, deterministic Web PWA feed/detail/stance/thread/moderation smokes, `public_feed_analysis_frame_reliability`, `public_feed_composition_freshness`, `public_feed_lifecycle_accountability`, `story_identity_growth`, `public_feed_pagination_refresh`, `stance_aggregate_decay_public_mesh`, the curated launch-content snapshot gate, the report-intake/admin-action smoke, the operator trust gate, the public-beta compliance gate, `luma_mvp_production_readiness`, and `public_beta_launch_closeout`. It writes `.tmp/mvp-release-gates/latest/mvp-release-gates-report.json` with per-gate `pass`, `fail`, `setup_scarcity`, or `skipped_not_in_scope` status. `pnpm check:luma:mvp-production-readiness`, `pnpm check:luma-forbidden-claims`, `pnpm check:luma-production-profile`, `pnpm check:luma-telemetry-redaction`, `pnpm check:public-feed:composition-freshness`, `pnpm check:public-feed:lifecycle-accountability`, `pnpm check:public-feed:stance-aggregate-decay`, `pnpm check:public-beta-compliance`, `pnpm check:public-beta-launch-control`, and `pnpm check:public-beta-launch-closeout` are also available as focused commands. | Week 3A release evidence harness and final closeout audit are present for the core news loop, public feed composition/freshness and analysis/frame reliability, curated fallback content, minimum report-to-action workflow, trusted beta operator gate, public-beta LUMA signed-write layer, public-beta policy surfaces, public support/contact path, and private support escalation protocol. A public-beta claim still requires a fresh packet on the release commit; broader ops/admin polish is classified as post-beta follow-up unless release copy claims production-grade live headline readiness. |
...
| Full-product multi-user engagement | PR #551 adds `pnpm live:stack:up:analysis-stub` and `pnpm test:live:five-user-engagement`. The stack starts Web PWA, local Gun relay, news daemon, StoryCluster, fixture feed, and deterministic chat-completions analysis stub; the Playwright lane creates five beta-local users, discovers two singleton and two bundled analysis-ready stories, verifies accepted `TopicSynthesisV2` frame/reframe detail, point-level stance mesh writes and aggregate readback, feed Eye/Lightbulb metrics, deterministic `news-story:*` threads, chained replies, and reload/cross-user visibility. | This is the current release-like manual QA lane for the full product loop. It supplements `pnpm check:mvp-release-gates` and does not prove live public-source freshness or production-grade headline density. |
...
- basic safety affordances exist: audited hide/restore moderation exists for story-thread comments, report intake can route story-thread reports to audited hide/restore actions, and those remediation writes require trusted beta operator authorization; user block UX, full RBAC/admin membership management, notifications/escalation, appeals, and broader moderation workflow polish remain open.
...
| 6 | `mvp-release-gates` | Complete; merged in PR #535 and hardened in PR #536; launch-content snapshot, report-intake/admin-action, operator-trust, and public-beta compliance gates added as follow-on closeout slices. | Deterministic feed/detail/stance/thread/correction/moderation, launch-content fallback, report-to-action, operator trust, and public-beta compliance release gates are named and report-backed. | `.tmp/mvp-release-gates/latest/mvp-release-gates-report.json` records command, artifact refs, timestamps, and pass/fail/setup-scarcity semantics for source health, story correctness, feed render, story detail, synthesis correction, point stance, story thread, story-thread moderation, `launch_content_snapshot`, `report_intake_admin_action`, `operator_trust_gate`, and `public_beta_compliance` gates. |
| 7 | `compliance-public-beta-minimums` | Complete for Web PWA beta policy surfaces, provisioned support/contact path, and minimum private escalation protocol. | Privacy, terms, UGC/moderation, support/contact, data deletion, telemetry/remote AI consent, content/copyright boundaries, and sensitive-case private handoff rules. | `/beta`, `/privacy`, `/terms`, `/moderation`, `/support`, `/data-deletion`, `/telemetry`, and `/copyright` exist and are linked from the app; `/support` links to the VHC public beta GitHub Issue Form backed by `.github/ISSUE_TEMPLATE/public-beta-support.yml`; deletion/correction, copyright/attribution, abuse/safety, and account/access cases have a public-safe issue stub plus operator private handoff protocol; `pnpm check:public-beta-compliance` verifies required routes, links, docs, support-channel wiring, no-overclaim language, and private-escalation coverage; `pnpm check:mvp-release-gates` includes `public_beta_compliance`. |
| 8 | `launch-ops-and-correction-path` | Complete for minimum public-beta remediation and fallback content; accepted synthesis correction merged in PR #537 and hardened in PR #538; story-thread comment hide/restore moderation merged in PR #539; curated validated snapshot fallback is implemented; minimum report intake, operator action queue, and trusted beta operator authorization gate are implemented. | Bad accepted synthesis, abusive story-thread comments, user report intake, and stale live-feed/demo scarcity now have minimum audited or deterministic fallback paths. Model/cost telemetry, richer admin UX, full RBAC/admin membership management, notification/escalation automation, and appeals are classified as post-beta follow-up unless the release copy claims those capabilities. | Operators have typed audit records for suppressing/unavailable accepted synthesis artifacts and hiding/restoring story-thread comments, plus a typed report queue that routes user reports to those existing actions or dismissal only when the current operator has a trusted beta operator capability record. Internal QA/demo has a committed curated fallback snapshot and `pnpm check:launch-content-snapshot`. `docs/ops/public-beta-launch-readiness-closeout.md` carries the remaining-work classification. |

Recommended sequencing:

- PR #527, PR #528, PR #530, PR #531, PR #532, PR #533, PR #535, PR #536, PR #537, PR #538, PR #539, PR #542, PR #543, PR #545, PR #546, PR #547, PR #548, and PR #549 are now in `main`; feed/detail stance/thread work can base on stable point ids, accepted publish-time synthesis, honest beta-local proof semantics, active personalization ranking, deterministic story discussion threads, release-gate evidence, accepted synthesis correction, story-thread comment hide/restore moderation, curated fallback launch content, minimum report-to-action admin workflow, public-beta policy pages, a provisioned public support/contact path, private escalation protocol, and trusted beta operator authorization for current remediation writes.
- Remaining work is classified in `docs/ops/public-beta-launch-readiness-closeout.md` as either `ship_blocker` or `post_beta_follow_up`. The core feed/detail/stance/thread product loop, minimum correction/moderation remediation paths, trusted beta operator gate for current remediation writes, report intake/admin action path, curated fallback launch content, public-beta policy surfaces, provisioned public support/contact path, private support escalation protocol, and final closeout evidence map have deterministic gate coverage.
...
| LUMA public-beta readiness | Go for MVP public-beta after the focused LUMA report passes. | `pnpm check:luma:mvp-production-readiness` writes `.tmp/luma-mvp-production-readiness/latest/luma-mvp-production-readiness-report.json` with `status: pass`, backed by current LUMA mesh reader-path coverage. | Do not claim production-attestation/Silver, public WSS mesh `release_ready`, or full production app readiness from this LUMA gate. |
...
| Compliance | Go for public beta policy surfaces, provisioned support/contact, and minimum private escalation protocol; legal review remains a separate release approval. | Privacy, terms, UGC/moderation, support/contact, data deletion, telemetry/remote AI consent, and content/copyright minimums have accepted drafts, user-facing routes, global links, a VHC public beta GitHub Issue Form support channel, public-safe issue stub rules, private handoff protocol for sensitive deletion/copyright/abuse/account cases, and `pnpm check:public-beta-compliance` / `public_beta_compliance` MVP-gate coverage. | No native App Store/TestFlight claim, legal approval claim, private support inbox claim, verified-human claim, automated escalation/SLA claim, or full trust-and-safety operations claim. |
...
| Correction/admin path | Go for minimum MVP remediation controls; accepted synthesis correction, story-thread comment hide/restore moderation, report intake/admin action queue, and trusted beta operator gate are implemented. | Users can report accepted synthesis artifacts and story-thread comments; trusted beta operators can dismiss reports or apply existing suppress/unavailable/hide/restore actions with typed audit metadata and `source_report_id` provenance; story detail hides stale summary/frame rows and moderated comment content; `pnpm check:mvp-release-gates` includes deterministic `synthesis_correction`, `story_thread_moderation`, `report_intake_admin_action`, and `operator_trust_gate` smokes. | Full RBAC/admin membership management, notification/escalation automation, appeals, and broader admin UX polish are `post_beta_follow_up`; do not imply a full trust-and-safety operations console exists. |
...
| Public-beta launch closeout | Go for engineering closeout when the release commit evidence packet passes. | `pnpm check:public-beta-launch-control` and `pnpm check:public-beta-launch-closeout` pass; `pnpm check:mvp-release-gates` includes `public_feed_analysis_frame_reliability`, `public_feed_composition_freshness`, `public_feed_lifecycle_accountability`, `story_identity_growth`, `public_feed_pagination_refresh`, `stance_aggregate_decay_public_mesh`, and `public_beta_launch_closeout`; and `docs/ops/public-beta-launch-control-2026-07-09.md` plus `docs/ops/public-beta-launch-readiness-closeout.md` map every named launch gate to a deterministic command/report path with remaining work classified as `ship_blocker` or `post_beta_follow_up`. | Do not make public-beta launch claims from a commit whose gate packet is missing, failing, or inconsistent with the documented beta scope. |
| Full-product multi-user engagement | Go as supplemental release-like QA. | `pnpm live:stack:up:analysis-stub` and `pnpm test:live:five-user-engagement` pass when the release note claims the full multi-user feed/detail/stance/thread loop was exercised against production-shaped local services. | Do not market the full multi-user product loop as exercised unless the five-user lane passes or the claim is removed. This lane does not replace deterministic release reports or live headline production-readiness evidence. |
...
| Operator trust-gate smoke | `pnpm check:mvp-release-gates` includes `operator_trust_gate` | Deterministic store/UI smoke proves current remediation writes fail closed without trusted beta operator authorization and succeed only with the required `review_news_report`, `write_synthesis_correction`, or `moderate_story_thread` capability. |
| LUMA public-beta readiness | `pnpm check:mvp-release-gates` includes `luma_mvp_production_readiness`; focused command is `pnpm check:luma:mvp-production-readiness` | Verifies beta-local AssuranceEnvelope runtime, stable device/session lifecycle, signed-write action policy, public namespace leak guards, current LUMA mesh reader-path coverage, and no Silver/verified-human/public-WSS/app-readiness overclaim. |
| iOS build | Missing because no iOS shell | Only required if Week 0 chooses iOS. |
| Privacy/UGC/deletion checklist | `pnpm check:mvp-release-gates` includes `public_beta_compliance`; focused command is `pnpm check:public-beta-compliance` | Required before public-beta claims; verifies `/beta`, `/privacy`, `/terms`, `/moderation`, `/support`, `/data-deletion`, `/telemetry`, and `/copyright` routes, global links, provisioned public support-channel wiring, release docs, and no-overclaim boundaries. |
| Public-beta launch closeout audit | `pnpm check:mvp-release-gates` includes `public_beta_launch_closeout`; focused commands are `pnpm check:public-beta-launch-control` and `pnpm check:public-beta-launch-closeout` | Verifies `docs/ops/public-beta-launch-control-2026-07-09.md` preserves an honest go/no-go packet and `docs/ops/public-beta-launch-readiness-closeout.md` maps every public-beta launch gate to deterministic command/report evidence with remaining work classified as `ship_blocker` or `post_beta_follow_up`. |
| Full-product five-user engagement live QA | Stack command `pnpm live:stack:up:analysis-stub`; test command `pnpm test:live:five-user-engagement` | Supplemental release-like validation for five beta-local users reading, taking point-level stances, and discussing singleton and bundled stories through the local mesh. Required only when release copy claims that full multi-user loop was exercised. |
...
Remaining work is classified in `docs/ops/public-beta-launch-readiness-closeout.md`: external legal/commercial approval is a `ship_blocker` only when required for the intended public distribution claim; regeneration workflow polish, full RBAC/admin membership management, notification/escalation automation, appeals, broader admin workflow UX, and model/cost operations visibility are `post_beta_follow_up` for the minimum Web PWA public beta.
...
- Deferred: production Sybil resistance;
