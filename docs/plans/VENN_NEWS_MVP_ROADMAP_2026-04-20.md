...
> Date: 2026-04-20
> Last alignment audit: 2026-07-11; current live decision delegated to `docs/ops/public-beta-operational-state.md`
...
| Scope A public-news operation | The raw-only publication contract, 2-of-3 relay REST quorum, graph diagnostics, staggered early heap capture/watchdog ceilings, interim email alerting, and watch closure exist. S1 remediation is merged at `3c8907f0`, but live recovery remains `NO_GO_STOP_REPORT_REMOTE_STAGING_BASE_UNSAFE` after attempt 001 stopped before mutation. Read `docs/ops/public-beta-operational-state.md` for moving truth. | Do not treat repo capability, a historical recovery, or relay readiness as sustained-readiness proof. S1 requires a newly reviewed private-staging envelope, serial A/B/C evidence, separate publisher recovery, and passing T0+48h closure before S2. Retention, publisher clear, eviction, relay compaction, custom pager cutover, and Codex live execution remain outside this gate. |
| Release gates | `pnpm check:mvp-release-gates` composes all 24 release IDs, including `source_health`, `story_correctness`, the deterministic Web PWA feed/detail/stance/thread/moderation smokes, `public_feed_analysis_frame_reliability`, `public_feed_composition_freshness`, `public_feed_lifecycle_accountability`, `public_feed_fresh_propagation`, `story_identity_growth`, `public_feed_pagination_refresh`, `stance_aggregate_decay_public_mesh`, launch-content/report/operator/compliance gates, `luma_forbidden_claims`, `luma_production_profile`, `luma_telemetry_redaction`, `luma_mvp_production_readiness`, and `public_beta_launch_closeout`. It writes `.tmp/mvp-release-gates/latest/mvp-release-gates-report.json` with per-gate `pass`, `fail`, `setup_scarcity`, or `skipped_not_in_scope` status. Focused public-feed, LUMA, compliance, launch-control, and closeout commands remain available. | Week 3A release evidence harness and final closeout audit are present for the core news loop, public feed composition/freshness and analysis/frame reliability, curated fallback content, minimum report-to-action workflow, trusted beta operator gate, public-beta LUMA signed-write layer, public-beta policy surfaces, public support/contact path, and private support escalation protocol. A public-beta claim still requires a fresh packet on the release commit; broader ops/admin polish is classified as post-beta follow-up unless release copy claims production-grade live headline readiness. |
