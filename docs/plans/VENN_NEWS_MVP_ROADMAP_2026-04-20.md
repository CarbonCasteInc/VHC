...
| Public-beta launch closeout | Go for engineering closeout when the release commit evidence packet passes. | `pnpm check:public-beta-launch-closeout` passes; `pnpm check:mvp-release-gates` includes `public_feed_analysis_frame_reliability`, `public_feed_composition_freshness`, `public_feed_lifecycle_accountability`, `story_identity_growth`, `public_feed_pagination_refresh`, `stance_aggregate_decay_public_mesh`, and `public_beta_launch_closeout`; and `docs/ops/public-beta-launch-readiness-closeout.md` maps every named launch gate to a deterministic command/report path with remaining work classified as `ship_blocker` or `post_beta_follow_up`. | Do not make public-beta launch claims from a commit whose gate packet is missing, failing, or inconsistent with the documented beta scope. |
...
| Public-beta launch closeout audit | `pnpm check:mvp-release-gates` includes `public_beta_launch_closeout`; focused command is `pnpm check:public-beta-launch-closeout` | Verifies `docs/ops/public-beta-launch-readiness-closeout.md` maps every public-beta launch gate to deterministic command/report evidence and classifies remaining work as `ship_blocker` or `post_beta_follow_up`. |
