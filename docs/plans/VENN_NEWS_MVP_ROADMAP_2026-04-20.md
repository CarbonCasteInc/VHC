...
> Date: 2026-04-20
> Last alignment audit: 2026-04-26 during private support escalation protocol branch from `main` at `0a13f5e5`
...
| Release gates | `pnpm check:mvp-release-gates` now composes source health, StoryCluster correctness, deterministic Web PWA feed/detail/stance/thread/moderation smokes, the curated launch-content snapshot gate, the report-intake/admin-action smoke, and the public-beta compliance gate. It writes `.tmp/mvp-release-gates/latest/mvp-release-gates-report.json` with per-gate `pass`, `fail`, `setup_scarcity`, or `skipped_not_in_scope` status. `pnpm check:public-beta-compliance` is also available as a focused command for policy route/docs/support-channel/private-escalation checks. | Week 3A release evidence harness is present for the core news loop, curated fallback content, minimum report-to-action workflow, public-beta policy surfaces, the public support/contact path, and private support escalation protocol; broader ops/admin polish remains open. |
...
| 8 | `launch-ops-and-correction-path` | Partial; accepted synthesis correction merged in PR #537 and hardened in PR #538; story-thread comment hide/restore moderation merged in PR #539; curated validated snapshot fallback is implemented; minimum report intake, operator action queue, and trusted beta operator authorization gate are implemented. | Model/cost telemetry, richer admin UX, full RBAC/admin membership management, and release artifact visibility remain open; bad accepted synthesis, abusive story-thread comments, and stale live-feed/demo scarcity now have minimum audited or deterministic fallback paths. | Operators have typed audit records for suppressing/unavailable accepted synthesis artifacts and hiding/restoring story-thread comments, plus a typed report queue that routes user reports to those existing actions or dismissal only when the current operator has a trusted beta operator capability record. Internal QA/demo has a committed curated fallback snapshot and `pnpm check:launch-content-snapshot`. Remaining launch-ops work must cover broader admin workflow UX, full RBAC/admin membership management, notification/escalation automation, appeals, and runaway model visibility. |

Recommended sequencing:

- PR #527, PR #528, PR #530, PR #531, PR #532, PR #533, PR #535, PR #536, PR #537, PR #538, PR #539, PR #542, PR #543, PR #545, PR #546, and PR #547 are now in `main`; feed/detail stance/thread work can base on stable point ids, accepted publish-time synthesis, honest beta-local proof semantics, active personalization ranking, deterministic story discussion threads, release-gate evidence, accepted synthesis correction, story-thread comment hide/restore moderation, curated fallback launch content, minimum report-to-action admin workflow, public-beta policy pages, and a provisioned public support/contact path.
- Broader admin workflow UX, full RBAC/admin membership management, legal review, notification/escalation automation, appeals, and ops/cost visibility are now the highest-value Week 0 blockers. The core feed/detail/stance/thread product loop, minimum correction/moderation remediation paths, trusted beta operator gate for current remediation writes, report intake/admin action path, curated fallback launch content, public-beta policy surfaces, provisioned public support/contact path, and private support escalation protocol have implementation and deterministic release-gate coverage.
...
| Correction/admin path | Go for minimum MVP remediation controls; accepted synthesis correction, story-thread comment hide/restore moderation, report intake/admin action queue, and trusted beta operator gate are implemented. | Users can report accepted synthesis artifacts and story-thread comments; trusted beta operators can dismiss reports or apply existing suppress/unavailable/hide/restore actions with typed audit metadata and `source_report_id` provenance; story detail hides stale summary/frame rows and moderated comment content; `pnpm check:mvp-release-gates` includes deterministic `synthesis_correction`, `story_thread_moderation`, `report_intake_admin_action`, and `operator_trust_gate` smokes. | Public launch still needs full RBAC/admin membership management, notification/escalation automation, appeals, and broader admin UX polish; do not imply a full trust-and-safety operations console exists. |
...
Still required: regeneration workflow polish, legal review, full RBAC/admin membership management, notification/escalation automation, appeals, broader admin workflow UX, and model/cost operations visibility.
