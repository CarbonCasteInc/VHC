...
- Topology lints and release gates:
  ...
  - News storyline system-writer migration covers `vh/news/storylines/<storylineId>` only: storyline nodes carry `_writerKind: 'system'` and validate with the shared system-writer validator, while the `vh/news/storylines/` root map, removal tombstones, latest/hot indexes, analysis, synthesis, discovery, and topic engagement migrate in later slices or remain out of scope as specified by their owning contracts.
