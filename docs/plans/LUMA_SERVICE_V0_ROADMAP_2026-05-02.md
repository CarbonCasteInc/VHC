...
- Topology lints and release gates:
  ...
  - New release gates: `pnpm check:public-namespace-leaks`, `pnpm check:linkability-domain-registry`, `pnpm check:luma-directory-v1`, `pnpm check:luma-forum-author-v1`, `pnpm check:luma-aggregate-voter-v1`, `pnpm check:luma-forum-post-v1`, `pnpm check:luma-news-report-v1`, `pnpm check:luma-forum-nomination-v1`, `pnpm check:luma-news-story-system-v1`, `pnpm check:luma-news-storyline-system-v1`, `pnpm check:luma-news-index-system-v1`.
  ...
  - News latest/hot index system-writer migration covers `vh/news/index/latest/<storyId>` and `vh/news/index/hot/<storyId>` child nodes only: child entries carry `_writerKind: 'system'` and validate with the shared system-writer validator, while index root maps, removal tombstones, analysis, synthesis, discovery, and topic engagement migrate in later slices or remain out of scope as specified by their owning contracts.
