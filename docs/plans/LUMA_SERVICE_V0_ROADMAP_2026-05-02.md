...
- Adapter and materializer migration (the surfaces the mesh hardening
  ...
  - `apps/web-pwa/src/store/newsReports.ts` and `packages/gun-client/src/newsReportAdapters.ts`: new report intake writes use `hermes-news-report-v2`, derived `forumAuthorId` in `reporter_id`, and `SignedWriteEnvelope.audience = 'vh-news-report'`; legacy `hermes-news-report-v1` records remain read/operator-action compatible only.
...
- Topology lints and release gates:
  ...
  - New release gates: `pnpm check:public-namespace-leaks`, `pnpm check:linkability-domain-registry`, `pnpm check:luma-directory-v1`, `pnpm check:luma-forum-author-v1`, `pnpm check:luma-aggregate-voter-v1`, `pnpm check:luma-forum-post-v1`, `pnpm check:luma-news-report-v1`, `pnpm check:luma-forum-nomination-v1`, `pnpm check:luma-news-story-system-v1`, `pnpm check:luma-news-storyline-system-v1`, `pnpm check:luma-news-index-system-v1`, `pnpm check:luma-news-analysis-system-v1`, `pnpm check:luma-topic-synthesis-system-v1`, `pnpm check:luma-topic-digest-system-v1`, `pnpm check:luma-topic-engagement-summary-system-v1`, `pnpm check:luma-civic-reps-system-v1`, `pnpm check:luma-discovery-index-system-v1`.
  - News bundle/story system-writer migration starts with `vh/news/stories/<storyId>`: story nodes carry `_writerKind: 'system'` and validate with the shared system-writer validator.
  - News storyline system-writer migration covers `vh/news/storylines/<storylineId>` only: storyline nodes carry `_writerKind: 'system'` and validate with the shared system-writer validator, while the `vh/news/storylines/` root map and removal tombstones remain out of scope.
  - News latest/hot index system-writer migration covers `vh/news/index/latest/<storyId>` and `vh/news/index/hot/<storyId>` child nodes only: child entries carry `_writerKind: 'system'` and validate with the shared system-writer validator, while index root maps and removal tombstones remain out of scope.
  - News analysis system-writer migration covers `vh/news/stories/<storyId>/analysis/<analysisKey>` and `vh/news/stories/<storyId>/analysis_latest` only: analysis artifacts and the analysis latest pointer carry `_writerKind: 'system'` and validate with the shared system-writer validator, while the analysis root map, `analysis_pending`, removal tombstones, discovery, and topic engagement migrate in later slices or remain out of scope as specified by their owning contracts.
  - The topic synthesis epoch/latest records system-writer migration covers `vh/topics/<topicId>/epochs/<epoch>/synthesis` and `vh/topics/<topicId>/latest` only: epoch/latest synthesis wrappers carry `_writerKind: 'system'` and validate with the shared system-writer validator, while candidates, corrections, digests, discovery indexes, topic engagement, and removal tombstones remain out of scope.
  - The topic digest records system-writer migration covers `vh/topics/<topicId>/digests/<digestId>` only: digest wrappers carry `_writerKind: 'system'` and validate with the shared system-writer validator, while candidates, corrections, discovery indexes, topic engagement, and removal tombstones remain out of scope.
  - The topic engagement summary system-writer migration covers `vh/aggregates/topics/<topicId>/engagement/summary` only: summary aggregates carry `_writerKind: 'system'` and validate with the shared system-writer validator, while topic-scoped actor nodes, aggregate voter/snapshot adapters, discovery indexes, mesh evidence artifacts, relay, and removal tombstones remain out of scope.
  - The civic representative directory snapshot system-writer migration covers `vh/civic/reps/<jurisdictionVersion>` only: representative directory snapshots carry `_writerKind: 'system'` and validate with the shared system-writer validator, while identity directory schemas, bridge action/receipt/stat adapters, discovery indexes, mesh evidence artifacts, relay, and browser signing remain out of scope.
  - The discovery item/index system-writer migration covers `vh/discovery/items/<topicId>` and `vh/discovery/index/<filter>/<sort>/<cursor>` only: public discovery records carry `_writerKind: 'system'` and validate with the shared system-writer validator, while app discovery stores/hooks/components, discovery root maps, tombstones/removal flows, mesh evidence artifacts, relay, browser signing, and private `MY_ACTIVITY` personalization remain out of scope.
...
- Mesh re-run gate (post-M0.B): schema-epoch revalidation only.
  ...
  - The post-M0.B mesh report MUST show, for each migrated write class:
    ...
    - daemon/system-published classes (`news bundle/story`, `storyline`, `news latest/hot index entry`, `story analysis artifacts`, `analysis latest pointer`, `topic synthesis epoch`, `synthesis latest pointer`, `topic digest`, `discovery indexes`, `civic representative directory snapshot`, `comment moderation/operator action`) have `drill_writer_kind_by_class[<class>] === 'system'` and pass `spec-data-topology-privacy-v0.md` §8 system-writer validation;
...
| `packages/gun-client/src/{forum,directory,aggregate,sentimentEvent,synthesis,news,newsReport,storyline,analysis,topicEngagement,civicRepresentative}Adapters.ts` | M0.B | M0.B owns adapter migration. M0.C does not modify adapter logic; M0.C may adjust import paths after M0.B has landed if the SDK surface is renamed. |
