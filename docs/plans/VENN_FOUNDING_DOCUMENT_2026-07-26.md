...
> Status: Founding Working Draft For The Successor Repository — Not Yet Ratified
> Current Custodian: VHC Core Engineering
> Last Reviewed: 2026-07-26
> Depends On: docs/foundational/trinity_project_brief.md, docs/specs/spec-civic-sentiment.md, docs/specs/spec-identity-trust-constituency.md

This document supersedes the 2026-07-25 Assent working draft. It consolidates the converged position from the 2026-07 strategy sequence: the landscape assessment, the Assent draft, the external critique of that draft, the instrument-thesis correction, the agent-era protocol synthesis, the structural privacy and governance corrections that followed, and the subsequent expansion from an assent instrument into a provider-neutral **Human-Signal Protocol**. Sections are written to be lifted into the successor repository as `CHARTER.md`, `CONSTITUTION.md`, a `spec/` skeleton, and `ROADMAP.md`.
...
Eight rounds of analysis converged here:
...
> **Venn is the open protocol for representing humanity in the age of AI. It turns everyday expression across the internet into typed, auditable measures of what participants care about and may believe, and into explicit measures of what they confirm — counting contributors under named personhood and deduplication policies rather than treating bots, posts, or noise as people. It gives platforms, agents, citizens, and institutions a shared public-opinion layer they can audit without publishing anyone's identity or political history.**

One protocol, many clients. The publisher embed, generic comment-system kit, native platform integration, Bluesky integration, share-to-agent flow, browser Lens, MCP server, verifier CLI, and future credential wallets are all _surfaces_ over the same record and measurement system. None of them is the product.
...
The predecessor's failure mode was scope. These are refusals, not deferrals. Changing one requires a charter amendment through the RFC process (§12.4).

1. **Not a news destination.** Venn does not compete for the "news app" slot. The reference layer is infrastructure that other surfaces embed.
...
5. **Not a binding-election system.** The core protocol produces advisory and pre-legislative evidence. A future ballot profile may reuse credential and ceremony standards only as a separately governed, legally authorized system with fresh ballot-specific consent, secret-ballot protections, and no conversion of an old sentiment into a vote.
...
8. **Not a social network or moderation platform.** Discourse happens on surfaces Venn does not operate; Venn supplies context and a standard way to interpret, confirm, and aggregate signal.
...
One jurisdiction supplies density, publisher cooperation, calibration, and a tractable methodology audit. The protocol supports plural jurisdictions, issuers, languages, methods, and thresholds from the first schema — the registries, not the code, carry jurisdictional difference.
...
### 10.5 What is sold — and never sold

Sold: aggregate civic intelligence, envelope verification, hosted private aggregation, instrumented consultations, calibration runs, transparent dashboards, credential/verifier adapters, platform SDK support, and conformance certification. **Never sold:** individual political profiles, readable per-person inference, raw cross-platform histories, or targeting segments. Those businesses are constitutionally prohibited and excluded by conformant custody and role boundaries; independent conformance, audits, and governance — not an impossibility claim — make the promise durable.
...
Before the public conformance mark activates, governance must establish:
...
- Build the reference artifact pipeline, schema package, resolver, envelope verifier, offline CLI, read-only API, mirrorable transparency log, generic host adapter, and comment-system SDK. Do not collect private inferred or confirmed political signal yet.
...
- Complete counsel/DPIA review and independent cryptographic/privacy review — including same-holder admission, outcome integrity, outcome-enumeration resistance, receipt deniability, and metadata leakage — before any live inferred or confirmed political-signal collection.
...
- Run role-separated private aggregation for bounded expression, salience, inferred, and confirmed contributions. The first production lane uses a narrow declared scope, independent aggregation operators, thresholding, metadata controls, and privacy-budget accounting.
...
- Establish the independent steward, appeals process, public funding/conflict record, conformance-mark licensing, and a recurring security/privacy/methodology review cycle.
...
- Explore a **separately chartered formal-ballot profile** only with election-law authorities, secret-ballot experts, accessibility communities, and public legitimacy. It receives no automatic scope inheritance from the opinion protocol, and no prior expression or position becomes a vote.
...
## 16. Open decisions requiring the human

These choices are intentionally not hidden in implementation defaults:

1. **Name and mark** — confirm `venn` as the protocol/repository name and complete a trademark and naming review before public launch.
2. **Stewardship transition** — choose the legal jurisdiction, initial selection process, funding firewall, and timetable for moving specification, registries, conformance suite, and mark into an independent nonprofit steward.
3. **First jurisdiction** — determines the public-record corpus, source-rights rules, legal basis, eligibility/place predicates, output thresholds, and first language/calibration benchmarks.
4. **V0 partners** — select at least one consuming institution and one publisher/community host; decide whether a platform/comment-system team joins as the third design partner.
5. **Political-data custody** — decide the first lane's acquisition basis, participant-local or ephemeral no-retention inference architecture, processor roles, retention/deletion periods, participant rights path, and independent audit scope.
6. **Credential-admission profile** — decide the minimum authorization, personhood, eligibility/place, deduplication, recovery, and accessibility requirements; choose the first optional providers without making any one provider normative.
7. **World ID posture** — decide whether to implement a World ID adapter in V2, later, or only through a community implementation. In every case it remains optional, version-pinned, provider-labeled, and independent of World App, Orb access, WLD, or World Chain.
8. **Cross-provider posture** — decide whether the first release publishes strictly separate provider cohorts (the safe default) or funds research into a privacy-preserving interoperability/deduplication construction. No product deadline may turn an unresolved overlap into an asserted headcount.
9. **Aggregation and privacy profile** — select independent deduplication/aggregation operators, threshold and abort rules, metadata controls, DP mechanism where appropriate, release/query budgets, and public compromise/recovery procedures.
10. **Counsel and DPIA gate** — complete before V2 political-signal collection, together with cryptographic, civil-rights, accessibility, and survey-method review.
11. **Formal-ballot boundary** — decide whether even to research a separately governed ballot profile during the first five years. The default is no binding-election claim or implementation.
12. **VHC repository disposition** — archive read-only with a pointer to the successor, or preserve explicitly named lanes as separate experiments. The old public-beta launch mission remains frozen either way; its operational state lives in `docs/foundational/STATUS.md`, not in this charter.
