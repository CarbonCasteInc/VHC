...
describe('clusterLifecycle identity coverage', () => {
  ...
  it('handles attachable documents with no retrieval entry from the vector backend', async () => {
    const document = makeWorkingDocument('doc-no-hit', 'Harbor disruption continues', 'hard_news');
    const next = await retrieveCandidates(makeState(makeTopicState('topic-no-hit'), [document]), {
    ...
    expect(next.documents[0]?.candidate_matches).toEqual([]);
    expect(next.documents[0]?.candidate_score).toBe(0);
    expect(next.stage_metrics.qdrant_candidate_retrieval?.candidates_considered).toBe(0);
  });
