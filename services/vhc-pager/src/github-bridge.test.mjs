...
import { GitHubIncidentBridge, incidentIssueBody } from './github-bridge.mjs';
  ...
  const bridge = new GitHubIncidentBridge({
    ...
    fetchImpl: async (url, init) => {
      ...
      if (url.includes('/issues?')) {
        return { ok: true, text: async () => JSON.stringify(calls.length > 1 ? [{ number: 9, body: 'Incident key: `a6:public-feed:exit_78`' }] : []) };
      }
