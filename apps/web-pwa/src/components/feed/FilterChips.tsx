...
const FilterChipButton: React.FC<FilterChipButtonProps> = ({
  ...
  return (
    <button
      ...
      className={
        isActive
          ? 'rounded-full border border-slate-900 bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow-sm dark:border-white dark:bg-white dark:text-slate-900'
          : 'rounded-full border border-transparent bg-transparent px-4 py-2 text-sm font-medium text-slate-600 transition hover:border-slate-200 hover:bg-white hover:text-slate-900 dark:text-slate-300 dark:hover:border-slate-700 dark:hover:bg-slate-800 dark:hover:text-white'
      }
    >
      {label}
    </button>
