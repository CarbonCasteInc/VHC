...
load_profile_env() {
  ...
  export VH_STORYCLUSTER_REMOTE_TIMEOUT_MS="${VH_STORYCLUSTER_REMOTE_TIMEOUT_MS:-300000}"
  ...
  export VH_NEWS_DAEMON_STATE_DIR="${VH_NEWS_DAEMON_STATE_DIR:-$ROOT/.tmp/live-local-stack/news-daemon-state}"
  ...
  if [[ "$STACK_MODE" == 'fixture' ]]; then
    ...
    if [[ "$STACK_MODE" == 'public' ]]; then
      ...
      export VH_STORYCLUSTER_REMOTE_TIMEOUT_MS="${VH_STORYCLUSTER_REMOTE_TIMEOUT_MS:-300000}"
      export VH_NEWS_RUNTIME_MAX_PUBLISHED_BUNDLES="${VH_NEWS_RUNTIME_MAX_PUBLISHED_BUNDLES:-32}"
      export VH_BUNDLE_SYNTHESIS_ENABLED="${VH_BUNDLE_SYNTHESIS_ENABLED:-true}"
      export VH_BUNDLE_SYNTHESIS_UPSTREAM_URL="${VH_BUNDLE_SYNTHESIS_UPSTREAM_URL:-$ANALYSIS_RELAY_UPSTREAM_URL}"
      export VH_BUNDLE_SYNTHESIS_API_KEY="${VH_BUNDLE_SYNTHESIS_API_KEY:-$ANALYSIS_RELAY_API_KEY}"
      export VH_BUNDLE_SYNTHESIS_MODEL="${VH_BUNDLE_SYNTHESIS_MODEL:-gpt-4o-mini}"
      export VH_BUNDLE_SYNTHESIS_TIMEOUT_MS="${VH_BUNDLE_SYNTHESIS_TIMEOUT_MS:-120000}"
      export VH_BUNDLE_SYNTHESIS_RATE_PER_MIN="${VH_BUNDLE_SYNTHESIS_RATE_PER_MIN:-120}"
      export VH_BUNDLE_SYNTHESIS_TEMPERATURE="${VH_BUNDLE_SYNTHESIS_TEMPERATURE:-0.2}"
      export VH_BUNDLE_SYNTHESIS_QUEUE_DEPTH="${VH_BUNDLE_SYNTHESIS_QUEUE_DEPTH:-512}"
      export VH_RELAY_WS_BYTES_PER_SEC="${VH_RELAY_WS_BYTES_PER_SEC:-8000000}"
    else
      unset VH_NEWS_FEED_MAX_ITEMS_PER_SOURCE
      unset VH_NEWS_FEED_MAX_ITEMS_TOTAL
      unset VH_STORYCLUSTER_REMOTE_MAX_ITEMS_PER_REQUEST
      unset VH_NEWS_RUNTIME_MAX_PUBLISHED_BUNDLES
      unset VH_BUNDLE_SYNTHESIS_ENABLED
      unset VH_BUNDLE_SYNTHESIS_UPSTREAM_URL
      unset VH_BUNDLE_SYNTHESIS_API_KEY
      unset VH_BUNDLE_SYNTHESIS_MODEL
      unset VH_BUNDLE_SYNTHESIS_TIMEOUT_MS
      unset VH_BUNDLE_SYNTHESIS_RATE_PER_MIN
      unset VH_BUNDLE_SYNTHESIS_TEMPERATURE
      unset VH_BUNDLE_SYNTHESIS_QUEUE_DEPTH
      unset VH_RELAY_WS_BYTES_PER_SEC
    fi
...
start_relay() {
  ...
  if [[ "$STACK_MODE" == 'public' && "${VH_LOCAL_STACK_RESET_RELAY:-false}" != 'true' ]]; then
    ...
    rm -rf \
      ...
      "$ROOT/data" \
      "$ROOT/radata" \
      "$ROOT/.tmp/live-local-stack/news-daemon-state" \
