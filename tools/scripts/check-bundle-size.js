...
async function main() {
  const [targetDir = 'apps/web-pwa/dist/assets', rawBudget = '1048576'] = process.argv.slice(2);
  ...
  for (const asset of jsAssets) {
    ...
    console.log(`Asset: ${path.basename(asset)} gzipped size: ${kb} KiB`);
    if (gzSize > budget) {
      violations += 1;
      console.error(`  Exceeds budget ${budget} bytes`);
    }
  }
