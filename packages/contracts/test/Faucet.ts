...
describe('Faucet', () => {
  async function deployFixture() {
    ...
    const minTrust = 5000;
    const faucet = await ethers.deployContract('Faucet', [await rgu.getAddress(), drip, cooldown, minTrust]);
    await faucet.waitForDeployment();

    await rgu.grantRole(await rgu.MINTER_ROLE(), await faucet.getAddress());
    await faucet.grantRole(await faucet.ATTESTOR_ROLE(), attestor.address);

    return { rgu, faucet, deployer, user, attestor, stranger, drip, cooldown, minTrust };
  }

  it('drips tokens for attested users and enforces cooldown', async () => {
    const { faucet, rgu, user, attestor, drip, cooldown } = await deployFixture();
