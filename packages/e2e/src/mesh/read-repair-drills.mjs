...
function drillChain(gun, runId, caseId, section, nodeId) {
  return gun.get('vh').get('__mesh_drills').get(runId).get('read_repair').get(caseId).get(section).get(nodeId);
}
...
function buildDrillRecord({ runId, traceId, caseDef, recordKind, writeId, attemptId, attemptOrdinal, state, issuedAt, expiresAt }) {
  ...
  const record = {
    schemaVersion: 'mesh-read-repair-drill-record-v1',
...
function findRepairSource({ sourceReadbacks, caseDef, section, nodeId }) {
  const candidates = sourceReadbacks
    .filter(
      (row) =>
        row.logical_key === caseDef.logicalKey &&
        row.section === section &&
        row.node_id === nodeId &&
        row.observed &&
        isCompleteDrillRecord(row.record, row.record?._drillRunId)
    )
    .sort((a, b) => {
      const relayOrder = { 'relay-a': 0, 'relay-c': 1 };
      return (relayOrder[a.relay_id] ?? 99) - (relayOrder[b.relay_id] ?? 99);
    });
  return candidates[0] || null;
}

async function repairCaseToRelay({ targetRelay, runId, caseDef, sourceReadbacks }) {
  const rows = [];
  for (const [section, nodeId] of nodesForCase(caseDef)) {
    const source = findRepairSource({ sourceReadbacks, caseDef, section, nodeId });
    if (!source?.record) {
      rows.push({
        case_id: caseDef.caseId,
        write_class: caseDef.objectClass,
        section,
        node_id: nodeId,
        source_relay_id: null,
        target_relay_id: targetRelay.relay_id,
        write_id: null,
        attempt_id: 'explicit-read-repair',
        ok: false,
        latency_ms: null,
        error: 'repair-source-missing',
      });
      continue;
    }
    const result = await putNode({
      peer: targetRelay.peerUrl,
      runId,
      caseId: caseDef.caseId,
      section,
      nodeId,
    ...
    rows.push({
      ...
      section,
      node_id: nodeId,
      source_relay_id: source.relay_id,
      target_relay_id: targetRelay.relay_id,
      write_id: source.record._drillWriteId,
      attempt_id: 'explicit-read-repair',
      ...result,
    });
  }
...
async function cleanupCaseAcrossRelays({ relays, runId, caseDef }) {
  let expected = 0;
  let cleaned = 0;
  const failures = [];
  for (const relay of relays) {
    const cleanup = await cleanupCase({ peer: relay.peerUrl, runId, caseDef });
...
function relayBRows(readbacks) {
  return readbacks.filter((row) => row.relay_id === 'relay-b');
}

function allRelayBRowsMissed(readbacks) {
  const rows = relayBRows(readbacks);
  return rows.length > 0 && rows.every((row) => !row.observed);
}

function repairEvaluation({ caseDef, preRepairRows, postRepairEvaluation, repairRows, repairStartedAt, repairCompletedAt }) {
  const targetPreRows = relayBRows(preRepairRows).filter((row) => row.logical_key === caseDef.logicalKey);
  const targetPostRows = postRepairEvaluation.per_relay.filter((row) => row.relay_id === 'relay-b');
  const sourceRelayIds = Array.from(new Set(repairRows.map((row) => row.source_relay_id).filter(Boolean)));
  const repairFailures = repairRows.filter((row) => !row.ok);
  const preRepairMiss = targetPreRows.length > 0 && targetPreRows.every((row) => !row.observed);
  const postRepairPass = postRepairEvaluation.status === 'pass';
  return {
    fixture: `${caseDef.caseId}-explicit-read-repair`,
    object_id: caseDef.objectId,
    object_class: caseDef.objectClass,
    logical_key: caseDef.logicalKey,
    canonical_id: caseDef.canonicalId,
    selected_strategy: 'explicit_read_repair',
    repair_source_relays: sourceRelayIds,
    repaired_relay_id: 'relay-b',
    expected_final_write_id: caseDef.writeId,
    pre_repair_miss: preRepairMiss,
    pre_repair_observed_rows: targetPreRows.filter((row) => row.observed).length,
    repair_attempts: repairRows.length,
    repair_successes: repairRows.filter((row) => row.ok).length,
    repair_failures: repairFailures,
    repair_latency_ms: repairCompletedAt - repairStartedAt,
    post_repair_direct_readback: targetPostRows,
    duplicate_count: postRepairEvaluation.duplicate_count,
    per_relay: postRepairEvaluation.per_relay,
    status: preRepairMiss && postRepairPass && repairFailures.length === 0 ? 'pass' : 'review_required',
    reason:
      preRepairMiss && postRepairPass && repairFailures.length === 0
        ? 'relay-b missed partition-period rows before repair, explicit read-repair replayed surviving-quorum records, and direct readback passed on every relay'
        : 'explicit read-repair did not produce complete direct readback evidence; inspect pre_repair_miss, repair_failures, and post_repair_direct_readback',
  };
}

async function runReadRepairDrill() {
  const startedAtMs = Date.now();
  const startedAt = new Date(startedAtMs).toISOString();
  const runId = makeId('mesh-read-repair');
  ...
  const proxies = [];
  const healthReasons = ['convergence-lagging'];
  let relays = [];
  try {
    ...
    proxies.push(aToB, bToA, cToB, bToC);
    ...
    const destroyedSocketCount = [aToB, bToA, cToB, bToC]
    ...
    for (const proxy of [aToB, bToA, cToB, bToC]) {
      proxy.setBlocked(false);
    }
    const healedAt = Date.now();
    await sleep(1500);

    const preRepairReadbacks = [];
    const preRepairEvaluations = [];
    const preRepairCaseReadbacks = await Promise.all(cases.map((caseDef) => readCaseFromRelays({
        relays,
        runId,
        caseDef,
        context: 'direct-single-relay-after-heal-before-explicit-read-repair',
        timeoutMs: Math.min(HEAL_CONVERGENCE_SLA_MS, READ_TIMEOUT_MS),
      })));
    for (const [index, rows] of preRepairCaseReadbacks.entries()) {
      const caseDef = cases[index];
      preRepairReadbacks.push(...rows);
      preRepairEvaluations.push(evaluateCase(caseDef, rows, relays.map((relay) => relay.relay_id)));
    }

    const repairStartedAt = Date.now();
    const repairWriteRows = [];
    for (const caseDef of cases) {
      repairWriteRows.push(
        ...(await repairCaseToRelay({
          targetRelay: relays[1],
          runId,
          caseDef,
          sourceReadbacks: [...partitionReadbacks, ...preRepairReadbacks],
        }))
      );
    }
    const repairCompletedAt = Date.now();
    await sleep(1000);

    const postRepairReadbacks = [];
    const postRepairEvaluations = [];
    const postRepairCaseReadbacks = await Promise.all(cases.map((caseDef) => readCaseFromRelays({
        relays,
        runId,
        caseDef,
        context: 'direct-single-relay-after-explicit-read-repair',
        timeoutMs: READ_TIMEOUT_MS,
      })));
    for (const [index, rows] of postRepairCaseReadbacks.entries()) {
      const caseDef = cases[index];
      postRepairReadbacks.push(...rows);
      postRepairEvaluations.push(evaluateCase(caseDef, rows, relays.map((relay) => relay.relay_id)));
    }
    ...
    const preRepairMissedRelayB = cases.every((caseDef) =>
      allRelayBRowsMissed(preRepairReadbacks.filter((row) => row.logical_key === caseDef.logicalKey))
    );
    const repairRowsByCase = Object.fromEntries(
      cases.map((caseDef) => [caseDef.caseId, repairWriteRows.filter((row) => row.case_id === caseDef.caseId)])
    );
    const repairEvaluations = cases.map((caseDef, index) =>
      repairEvaluation({
        caseDef,
        preRepairRows: preRepairReadbacks,
        postRepairEvaluation: postRepairEvaluations[index],
        repairRows: repairRowsByCase[caseDef.caseId] || [],
        repairStartedAt,
        repairCompletedAt,
      })
    );
    const failedRepairEvaluations = repairEvaluations.filter((row) => row.status !== 'pass');

    let cleanupExpected = 0;
    let cleanupCleaned = 0;
    const cleanupFailures = [];
    for (const caseDef of [baselineCase, ...cases]) {
      const cleanup = await cleanupCaseAcrossRelays({ relays, runId, caseDef });
    ...
    if (failedPostRepairEvaluations.length > 0 || failedRepairEvaluations.length > 0 || !preRepairMissedRelayB) {
      healthReasons.push('write-readback-failed');
    }
    if (isolatedRelayObservedDuringPartition) healthReasons.push('convergence-lagging');
    if (!cleanupPassed) healthReasons.push('write-readback-failed');

    const resourceSlos = await collectMetrics(relays);
    const completedAtMs = Date.now();
    const repairLatencyMs = repairCompletedAt - repairStartedAt;
    const explicitRepairPassed =
      baselinePassed &&
      writeFailures.length === 0 &&
      partitionLiveFailures.length === 0 &&
      repairFailures.length === 0 &&
      failedPostRepairEvaluations.length === 0 &&
      failedRepairEvaluations.length === 0 &&
      preRepairMissedRelayB &&
    ...
    const drillCompletedTruthfully =
      ...
      !isolatedRelayObservedDuringPartition &&
      cleanupPassed;
    const gateResultStatus = explicitRepairPassed ? 'pass' : drillCompletedTruthfully ? 'review_required' : 'blocked';
    const allReadbacks = [
      ...baselineReadbacks,
      ...partitionReadbacks,
    ...
    const report = {
      ...
      run: {
        mode: 'local_read_repair_strategy',
      ...
      status_reason: explicitRepairPassed
        ? 'Slice 9B explicit read-repair strategy passed for synthetic mesh drill records; full production readiness remains review_required because automatic partition-heal, soak, evidence scrub, full clock-skew command, and post-M0.B LUMA-gated sections remain pending.'
        : drillCompletedTruthfully
          ? 'Slice 9B bounded local read-repair evidence completed truthfully, but explicit repair did not fully pass; inspect read_repair_drills, per_relay_readback, and health reasons.'
          : 'Slice 9B read-repair evidence failed or cleanup did not complete; inspect read_repair_drills, per_relay_readback, and health reasons.',
      ...
      topology: {
        ...
        selected_strategy: 'explicit_read_repair',
        selected_strategy_scope: 'synthetic_mesh_drill_records_only',
        ...
        relay_to_relay_auth_negative_test_reason: 'covered by pnpm test:mesh:topology-drills; Slice 9B only controls local/private relay-peer reachability for explicit read-repair evidence',
        partition_controls: proxies.map((proxy) => proxy.stats()),
        destroyed_socket_count_at_partition_start: destroyedSocketCount,
        peer_config_id: `local-three-relay-read-repair-${runId}`,
        ...
        read_repair: {
          ...
          pre_repair_miss_observed: preRepairMissedRelayB,
          post_repair_direct_readback_passed: failedPostRepairEvaluations.length === 0,
        },
      },
      gates: [
        {
          name: 'local-explicit-read-repair-drill',
          status: drillCompletedTruthfully ? 'pass' : 'fail',
          result_status: gateResultStatus,
          command: 'pnpm test:mesh:read-repair-drills',
          duration_ms: completedAtMs - startedAtMs,
          exit_code: drillCompletedTruthfully ? 0 : 1,
          reason: explicitRepairPassed
            ? 'relay-b missed partition-period rows before repair, explicit read-repair replayed surviving-quorum records, post-repair direct readback passed, and cleanup completed'
            : drillCompletedTruthfully
              ? 'bounded drill completed, but explicit read-repair evidence is review_required'
        ...
        },
        {
          ...
          duration_ms: 0,
          exit_code: null,
          reason: 'standalone automatic partition/heal proof remains owned by pnpm test:mesh:partition-drills and is run separately as a regression gate',
        },
        {
      ...
      write_class_slos: cases.map((caseDef, index) => {
        const row = repairEvaluations[index];
        const writes = [...allWriteResults, ...repairWriteRows].filter((write) => write.case_id === caseDef.caseId);
      ...
      peer_failure_drills: [
        ...
        {
          name: 'pre-repair-relay-b-direct-readback-miss',
          status: preRepairMissedRelayB ? 'pass' : 'review_required',
          reason:
            preRepairMissedRelayB
              ? 'relay-b did not directly observe partition-period rows after heal before explicit repair'
              : 'relay-b observed at least one partition-period row before explicit repair, so the repair gap was not cleanly exercised',
          checked_relay_id: 'relay-b',
        },
        {
          name: 'post-repair-direct-single-relay-readback',
          status: failedPostRepairEvaluations.length === 0 ? 'pass' : 'review_required',
          reason:
            failedPostRepairEvaluations.length === 0
              ? 'all relays directly observed canonical/index/projection records after explicit read-repair'
              : 'one or more relays did not directly observe complete post-repair records',
          repair_latency_ms: repairLatencyMs,
        },
      ],
      partition_heal_drills: preRepairEvaluations.map((row) => ({
        fixture: row.fixture,
        trace_id: traceId,
        status: row.status === 'pass' ? 'pass' : 'review_required',
        reason:
          row.status === 'pass'
            ? 'automatic heal unexpectedly converged before explicit repair'
            : 'automatic heal did not produce complete direct relay-b readback before explicit repair',
      ...
      read_repair_drills: repairEvaluations,
      state_resolution_drills: [
        {
          ...
          status: 'skipped',
          reason: 'Slice 7C state-resolution matrix remains covered by pnpm test:mesh:state-resolution-drills; Slice 9B only verifies explicit repair of synthetic partition-period drill records.',
        },
      ],
      conflict_fixtures: [
        ...repairEvaluations.map((row) => ({
          fixture: row.fixture,
          trace_id: traceId,
      ...
      clock_skew: {
        ...
        lww_diverged: false,
        status: 'skipped',
        reason: 'clock-skew classification remains covered by pnpm test:mesh:partition-drills; Slice 9B is scoped to explicit read-repair strategy evidence.',
      },
      ...
      cleanup: {
        namespace: `vh/__mesh_drills/${runId}/read_repair/*`,
      ...
      release_claims: {
        allowed: explicitRepairPassed
          ? [
              'The bounded local three-relay harness directly observed relay-b missing synthetic partition-period rows before explicit repair.',
              'The bounded local three-relay harness repaired relay-b by replaying surviving-quorum synthetic drill records from direct A/C readback.',
              'The bounded local three-relay harness directly observed all repaired synthetic drill records on relay-b after explicit read-repair.',
            ]
          : drillCompletedTruthfully
            ? [
                'The bounded local three-relay harness directly observed a real isolated-relay partition after an all-live baseline.',
                'The bounded local three-relay harness directly observed remaining-quorum synthetic writes during the partition.',
                'The bounded local drill attempted explicit read-repair and recorded review_required evidence.',
              ]
            : [],
        forbidden: [
          'Public WSS infrastructure is partition/heal production-ready.',
          'Automatic partition-heal convergence is proven.',
          'Explicit read-repair is proven for production LUMA-gated write classes.',
      ...
      downstream_canary: {
        ...
        status: 'skipped',
        reason: 'full downstream production-readiness gate is not wired in Slice 9B',
      },
      topology_strategy_review: {
        status: explicitRepairPassed ? 'selected_strategy_drilled' : 'required',
        selected_strategy: 'explicit replication/read-repair',
        reason: explicitRepairPassed
          ? 'explicit read-repair closed the bounded synthetic relay-b post-heal miss using direct surviving-quorum readback; automatic peer-federation recovery remains unclaimed'
          : 'explicit read-repair did not fully close the bounded synthetic relay-b post-heal miss',
        candidate_strategies: explicitRepairPassed
          ? []
          : [
              'tighten explicit replication/read-repair',
              'scoped Gun/AXE topology',
              'authoritative relay cluster with narrower failover claim',
            ],
        production_claim_boundary: 'synthetic mesh drill records only; no LUMA-gated write classes and no automatic partition-heal claim',
      },
    };
      ...
      JSON.stringify(
        {
          ...
          explicit_read_repair_passed: explicitRepairPassed,
          relay_b_pre_repair_miss: preRepairMissedRelayB,
          repair_latency_ms: repairLatencyMs,
          duplicate_counts: repairEvaluations.map((row) => ({
            fixture: row.fixture,
            duplicate_count: row.duplicate_count,
            status: row.status,
          })),
...
main()
  ...
  .catch((error) => {
    console.error(`[vh:mesh-read-repair-drills] fatal: ${error instanceof Error ? error.stack ?? error.message : String(error)}`);
    process.exit(1);
  });
