...
describe('system writer validation foundation', () => {
  ...
  it('accepts only the explicit system writer path matrix', () => {
    ...
    expect(isSystemWriterAllowedPath('vh/discovery/index/latest')).toBe(false);
    expect(isSystemWriterAllowedPath('vh/discovery/index/ALL/MY_ACTIVITY/page-1')).toBe(false);
    expect(isSystemWriterAllowedPath('vh/discovery/index/PRIVATE/LATEST/page-1')).toBe(false);
    expect(isSystemWriterAllowedPath('vh/discovery/private/local')).toBe(false);
